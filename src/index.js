...
function transformLiquid(ast, { env }) {
  ...
  visit(ast, {
    ...
    TextNode(node, _parent, _key, _index, meta) {
      ...
      node.source =
        node.source.slice(0, node.position.start) +
        node.value +
        node.source.slice(node.position.end);
    ...
    String(node, _parent, _key, _index, meta) {
      ...
      node.source =
        node.source.slice(0, node.position.start + 1) +
        node.value +
        node.source.slice(node.position.end - 1);
      meta.sourceNode.source = node.source;
    },
  });
}
...
function transformJavaScript(ast, { env }) {
  visit(ast, {
    JSXAttribute(node) {
      ...
      if (['class', 'className', 'style', 'tailwind'].includes(node.name.name)) {
